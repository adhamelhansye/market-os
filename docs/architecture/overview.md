...
Phase 7 adds a deterministic **Strategize** brick: structured positioning,
offer and messaging artifacts plus multi-perspective candidate decisions,
all derived from stored research and economics with explicit
classification, strength, provenance and versioned snapshots — never
generated by, or attributed to, an LLM (see `docs/architecture/strategy.md`).
