...
class CreativeBrief(Base):
    ...
    proof: Mapped[list[dict[str, Any]]] = mapped_column(JSONB, nullable=False, default=list)
    objection: Mapped[str | None] = mapped_column(String(50), nullable=True)
    cta: Mapped[str | None] = mapped_column(String(50), nullable=True)
...
class CreativeProvenance(Base):
    ...
    __table_args__ = (
        # CHECK constraint: exactly one of positioning_id / offer_id /
        # messaging_id / funnel_id must be NOT NULL.
        sa.CheckConstraint(
...
class CreativeStrategy(Base):
    ...
    __table_args__ = (
        Index("ix_creative_strategies_business_created", "business_id", "created_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    ...
    research_reference: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("research_projects.id", ondelete="RESTRICT"), nullable=True
    )
    strategy_decision_reference: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("strategy_decisions.id", ondelete="RESTRICT"), nullable=True
    )
    creative_intelligence_reference: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("creative_concepts.id", ondelete="RESTRICT"), nullable=True
    )
...
class CreativeTestVariant(Base):
    ...
    __table_args__ = (
        Index("ix_creative_test_variants_test_id", "test_id"),
        Index("ix_creative_test_variants_business_created", "business_id", "created_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    ...
    control_state_frozen_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
...
class CreativePortfolio(Base):
    ...
    __table_args__ = (
        Index("ix_creative_portfolios_business_created", "business_id", "created_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    ...
    )  # core / exploration / proof / objection / offer / format / angle
...
class CreativeCoverage(Base):
    ...
    __table_args__ = (
        Index("ix_creative_coverage_business_created", "business_id", "created_at"),
        Index("ix_creative_coverage_business_type", "business_id", "coverage_type"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    ...
    evidence_ref: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("creative_evidence.id", ondelete="RESTRICT"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
...
class CreativeDiversity(Base):
    ...
    __table_args__ = (
        Index("ix_creative_diversity_business_created", "business_id", "created_at"),
        Index("ix_creative_diversity_business_type", "business_id", "risk_type"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
...
class CreativeConceptPortfolio(Base):
    ...
    __table_args__ = (
        Index("ix_creative_concept_portfolios_concept_id", "creative_concept_id"),
        Index("ix_creative_concept_portfolios_business_created", "business_id", "created_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
    ...
    creative_concept_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("creative_concepts.id", ondelete="CASCADE"), nullable=False
    )
    portfolio_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("creative_portfolios.id", ondelete="CASCADE"), nullable=False
    )
...
class CreativeStrategySnapshot(Base):
    ...
    __table_args__ = (
        Index("ix_creative_strategy_snapshots_business_created", "business_id", "created_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False
