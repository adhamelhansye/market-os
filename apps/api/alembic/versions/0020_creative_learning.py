"""Creative learning snapshots (Phase 8D).

Creates one table:

- creative_learning_snapshots
  Immutable, fingerprint-keyed learning reports computed deterministically
  from Phase 8C performance observations.
...
def downgrade() -> None:
    op.drop_index(
        "ix_creative_learning_snapshots_business_created",
        table_name="creative_learning_snapshots",
    )
    op.drop_index(
        "uq_creative_learning_snapshots_fingerprint",
        table_name="creative_learning_snapshots",
    )
    op.drop_table("creative_learning_snapshots")
